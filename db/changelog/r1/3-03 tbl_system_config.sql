...
comment on table  system_config is 
  'Platform configuration registry. Read via PKG_CONFIG only (typed, cached). Secrets are NEVER stored 
here (TDS 16.5).'; 
comment on column system_config.config_code is 'Dot-namespaced key, e.g. AUTH.MAX_FAILED_LOGINS.'; 
comment on column system_config.data_type   is 'TEXT/NUMBER/FLAG/JSON - PKG_CONFIG validates writes and 
converts reads.'; 
comment on column system_config.is_tenant_overridable_yn is 'Y = TENANT_CONFIG may override this key.'; 
comment on column system_config.is_sensitive_yn is 'Y = value masked in UI/exports (still not for 
secrets).';
