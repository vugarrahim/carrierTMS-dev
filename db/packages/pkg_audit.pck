...
create or replace package body pkg_audit as
 ...
 begin
 return upper(p_field) in
 ('PASSWORD_HASH','PASSWORD_SALT','PASSWORD_ITERATIONS',
 'MFA_SECRET','TOKEN_HASH');
 end;
 ...
 procedure write_row (p_entity_code  in varchar2,
  ...
  begin
    ...
    insert into audit_log
      (tenant_id, user_id, action_code, entity_code, entity_id,
       entity_label, field_changes, context_info)
    values
      (to_number(sys_context('app_ctx','tenant_id')),
       to_number(sys_context('app_ctx','user_id')),
       upper(p_action_code), upper(p_entity_code), p_entity_id,
 ...
 begin
 write_row(p_entity_code, p_entity_id, p_action_code, p_entity_label, null);
 end;
 ...
 l_o json_object_t;
 begin
 if g_changes is null then chg_init; end if;
 -- Record only real changes; NULL-safe comparison.
 if (p_old is null and p_new is null)
 or p_old = p_new then
 return;
 end if;
 l_o := json_object_t();
 ...
 write_row(p_entity_code, p_entity_id, p_action_code,
 p_entity_label, g_changes);
 g_changes := null; -- always reset collector state
 end;
 procedure log_security_event (p_action_code in varchar2,
 p_detail in varchar2 default null) is
 begin
 write_row('SECURITY', null, p_action_code, substr(p_detail, 1, 200), null);
 end;
end pkg_audit;
/
